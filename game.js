...
function checkCollisions() {
    gameState.balls.forEach((ball) => {
        gameState.hands.forEach((hand) => {
            ...
            // Check if ball is collising with hand zone
            if (distance < ball.radius + config.handRadius) {
                ...
                const targetX = hand.x + Math.cos(angle) * (ball.radius + config.handRadius);
                const targetY = hand.y + Math.sin(angle) * (ball.radius + config.handRadius);
                ball.x = targetX;
                ball.y = targetY;
            }
        });
